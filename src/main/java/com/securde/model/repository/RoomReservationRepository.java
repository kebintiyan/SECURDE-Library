...
public interface RoomReservationRepository extends CrudRepository<RoomReservation, Integer> {
    ...
    @Query("SELECT rr " +
            "FROM RoomReservation rr " +
            "WHERE rr.reservationDate >= :date")
    ArrayList<RoomReservation> findRoomReservationsFromDate(@Param("date") String date);
    ...
    @Query("SELECT rr " +
            ...
            "rr.reservationDate >= :date")
    ArrayList<RoomReservation> findRoomReservationsByRoomIdFromDate(@Param("roomId") Integer roomId, @Param("date") String date);
